...
class Tasks(ArtemisCog):
    ...
    async def update_submission_channel(self) -> None:
        ...
        submission_channel: discord.TextChannel | discord.abc.GuildChannel | None = guild.get_channel(config.submission_channel_id)
        if not isinstance(submission_channel, discord.TextChannel):
            raise RuntimeError(f'The submission channel configured is of {type(submission_channel)} type, not TextChannel! Aborting task!')
